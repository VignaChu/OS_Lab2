...
# Challenge2：SLUB 小对象分配器（需要编程）

## 主要思想
在现有“页分配器”上再加一层更细的“按规格装箱”，让 `kmalloc/kfree` 处理小对象又快又省心。

为什么要加这一层：buddy/best-fit 直接按字节去要页，遇到一堆 几十到几百字节 的分配会很浪费、也容易碎片化；而 SLUB 的套路就是“同尺寸放一起”，把零散的小块装进固定大小的盒子里。

我们的做法（核心）：
- **按尺寸分班（size-class）**：8/16/32/…/2048B，统一**向上取整并 8B 对齐**。申请 N 字节，就进“最小能装下 N 的班”。
- **每个班有一堆 slab（默认 1 页）**：页首放一个 `slab_hdr` 记录元数据，后面是一排定长对象，末尾用**位图**标记哪些格子被占了。
- **分配路径（≤1 页）**：优先从该班的 *partial* 链表拿一个 slab，在位图里找第一个 0 置 1，直接返回对象指针；如果没有可用的 slab，就向页分配器再要 1 页建新 slab。
- **释放路径**：根据指针反推回它属于哪个 slab，位图清 0，计数更新；如果这个 slab 彻底空了，就把**整页还给页分配器**，及时止损外碎片。
- **大块（>1 页）**：不走 SLUB，直接把活甩给页分配器，避免把超大对象塞进小格子里搞复杂度。
- **可观测&自检**：实现 `slub_dump_stats` / `slub_check_invariants`，方便打印各班使用情况和不变量。
